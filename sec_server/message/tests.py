...
class Tests(TestCase):
    ...
    def setUp(self):
        ...
        self.message = self.thread.message_set.all()[0]
    
    def test_perm(self):
        self.assertEqual(can_get_message(self.c_user, self.message.id), True)
        self.assertEqual(can_get_message(self.c_user_1, self.message.id), False)
    
    def test_get_message(self):
        self.assertEqual(self.client.get(f'/message/{self.message.id}/').status_code, 200)
        self.assertEqual(self.client2.get(f'/message/{self.message.id}/').status_code, 401)

    def test_reply_all(self):
        self.assertEqual(self.client.post(f'/message/{self.message.id}/', content_type='application/json', data = {'message':'test message', 'subject':'test subject'}).status_code, 200)
        self.assertEqual(self.client2.post(f'/message/{self.message.id}/', content_type='application/json', data = {'message':'test message', 'subject':'test subject'}).status_code, 401)

    def test_reply_author(self):
        self.assertEqual(self.client.put(f'/message/{self.message.id}/', content_type='application/json', data = {'message':'test message', 'subject':'test subject'}).status_code, 200)
        self.assertEqual(self.client2.put(f'/message/{self.message.id}/', content_type='application/json', data = {'message':'test message', 'subject':'test subject'}).status_code, 401)
